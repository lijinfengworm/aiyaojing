...
class MY_Controller extends CI_Controller{
    ...
    private function _formatTree($data, $tmpUserAuth){
        ...
        foreach($data as $v){
            ...
            if($v['fid'] == 0 ){
                if(!empty($tmpUserAuth) && in_array($v['id'], $tmpUserAuth) && $this->userData['account'] != 'admin') {
                    $tmpTree[$v['id']] = $v;
...
class AdminUser {
    ...
    public static function buildFromSession($userData) {
        if(!empty($userData['uid'])) {
            return new AdminUser($userData['uid'], $userData['account'], $userData['realname']);
        }
        return;
    }
